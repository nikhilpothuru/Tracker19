...
with open('csvFiles/stateData/bnoStateData.csv', 'w') as csv_file:
    ...
    for i in range(57):
        ...
        if(i == 202):
            continue
        stateName = tbody.contents[i+5].contents[1].get_text()
        cases = tbody.contents[i+5].contents[2].get_text()
        newCases = tbody.contents[i+5].contents[3].get_text()
        deaths = tbody.contents[i+5].contents[4].get_text()
        newDeaths = tbody.contents[i+5].contents[5].get_text()
        deathRate = tbody.contents[i+5].contents[6].get_text()
        seriousCritical = tbody.contents[i+5].contents[7].get_text()
        recovered = tbody.contents[i+5].contents[8].get_text()
        # print(recovered)
